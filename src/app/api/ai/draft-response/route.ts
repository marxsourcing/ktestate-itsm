...
요청 정보:
- 제목: ${requestTitle}
- 유형: ${typeLabels[requestType] || requestType}
...
`.trim()
    ...
    if (!geminiApiKey) {
      ...
      return NextResponse.json({
        draft: generateDummyDraft(requesterName, requestTitle, requestType, typeLabels)
      })
    }
...
function generateDummyDraft(
  ...
  const type = typeLabels[requestType] || '요청'
...
${type} 요청으로 접수되어 담당자가 내용을 검토 중에 있습니다.
